...
import { Gender, Role, Status, User, UserCondDTO, userCondDTOSchema, UserLoginDTO, UserLoginDTOSchema, UserRegistrationDTO, UserRegistrationDTOSchema, UserUpdateDTO, userUpdateDTOSchema } from "../model";
...
export class UserUseCase implements IUserUseCase {
  ...
  async verifyToken(token: string): Promise<TokenPayload> {
    ...
    return { sub: user.id, role: user.role as any };    
  }
  ...
  async login(data: UserLoginDTO): Promise<string> {
    ...
    const token = jwtProvider.generateToken({sub: user.id, role: role});
    return token;
  }
